    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>家庭聚会</span><span>/家庭聚会</span>
         </div>
        </div>
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button v-show="rootObj.insert" style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增婚礼用品</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.weddingName }}</td>
        ...
        <p style="display:none">所在城市</p>
        <div style="width:100%; overflow: hidden; margin-top:8px; display:none">
                    <span style="border:1px solid #cccccc; float:left; height:38px; width:17%; line-height: 38px;text-align: center">选择城市</span>
                    <el-select  @change="changeCity()" v-model="textContent2" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
        ...
        <p style="display:none">二级城市</p>
        <div style="width:100%; overflow: hidden; margin-top:8px;display:none">
                    <span style=" border:1px solid #cccccc; float:left; height:38px; width:17%; line-height: 38px;text-align: center">选择城市</span>
                    <el-select  v-model="textContent6" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
        ...
        <div style="width:100%; overflow: hidden; margin-top:8px;">
                    <span style="border:1px solid #cccccc; float:left; height:38px; width:17%; line-height: 38px;text-align: center">选择分格</span>
...
export default {
 ...
 /****************************查看详情over********************************************************/             
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.waCheckheadline, {
                        weddingName: this.title,
                        id:blurId
                        }).then(data=> {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.waUpdateWedding+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.waUpdateWedding+"?token="+getCookie("token");
                 console.log(this.list[index].weddingName)
                 console.log(this.list[index].weddingIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].weddingName;
                let contentAll = this.list[index].weddingIntroduce.split('`');
                this.textContent1=contentAll[0];
                this.textContent2=contentAll[1].split("-")[0];
                this.textContent3=contentAll[2];
                this.textContent4=contentAll[3];
                this.textContent5=contentAll[4];
                this.textContent6=contentAll[1].split("-")[1];
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.waInsertWedding+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.waInsertWedding+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
            ...
            //如果是修改模块，不进行验证上传的图片是否为空     
                ...
                if(!this.title || !this.textContent1 || !this.textContent3 || !this.textContent4 || !this.textContent5){
                    ...
                    console.log("this.textContentAll",this.textContent1+this.textContent2+this.textContent3+this.textContent4+this.textContent5+this.textContent6)
                   
                    return 
                }
                this.pppss.weddingName=this.title
                this.pppss.weddingIntroduce=this.textContent1+"`"+"1城市(隐藏)"+"-"+"2城市(隐藏)"+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5
                ...
                if(this.submitName=="确认修改"){
                    ...
                    postRequest(url.waUpdateWedding, {
                                id:this.list[index].id,
                                weddingName:this.title,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.waDeleteWedding, {
                              pageNum: this.currentPage, 
                                weddingId:this.list[index].id
                                }).then(data=> {
    ...
    //    console.log(this.checkedCities,"打印的checkedCities值")
      ...
      handleCheckedCitiesChange(value) {
        ...
        this.isIndeterminate = checkedCount > 0 && checkedCount < this.list.length;
            ...
            goSearch(){//搜索
                postRequest(url.waFindAllWedding, {
                   keyword: this.search,
                }).then(data=> {
                       console.log("打印的数据",data.data.data)
     ...
     created:function(){
                ...
                postRequest(url.dsFindAllDictionaries, {
                    ...
                    type:"C-HY"
                            ...
                            console.log("select分类打印的数据111111111111",data.data.data)
                            let datas=data.data;
