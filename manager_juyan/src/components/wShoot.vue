    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>四大金刚</span><span>/摄影</span>
         </div>
        </div>
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增摄影</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.cameraName }}</td>
                <td style="text-align: center;">{{item.cameraIntroduce }}</td>
...
export default {
 ...
 /****************************查看详情over********************************************************/             
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.caCheckCameraName, {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.caUpdateCamera+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.caUpdateCamera+"?token="+getCookie("token");
                 console.log(this.list[index].cameraName)
                 console.log(this.list[index].cameraIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].cameraName;
                let contentAll = this.list[index].cameraIntroduce.split('`');
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.caInsertCamera+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.caInsertCamera+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
            ...
            //如果是修改模块，不进行验证上传的图片是否为空     
                ...
                this.pppss.cameraName=this.title
                // this.pppss.cameraIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5+"`"+this.textContent6
                this.pppss.cameraIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5
                ...
                if(this.submitName=="确认修改"){
                    ...
                    postRequest(url.caUpdateCamera, {
                                id:this.list[index].id,
                                cameraName:this.title,
                                cameraIntroduce:this.pppss.cameraIntroduce,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.caDeleteCamera, {
                                 pageNum: this.currentPage, 
                                cameraId:this.list[index].id
                                }).then(data=> {
           ...
           //控制checkbox复选框
            ...
            goSearch(){//搜索
                postRequest(url.caFindAllCamera, {
                   keyword: this.search,
                }).then(data=> {
                       console.log("打印的数据",data.data.data)
