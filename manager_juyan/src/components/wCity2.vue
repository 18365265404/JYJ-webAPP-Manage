    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>城市管理</span><span>/县城（区）管理</span>
         </div>
        </div>
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:15px;">
            ...
            <el-button v-show="rootObj.insert" style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增城市</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
		    ...
		    <tr class="thead-tr">
              ...
              <th style="text-align: center;">县区名称</th> 
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.cityName }}</td>
                 <td style="text-align: center;">{{item.countiesName}}</td>
        ...
        <div style="width:100%; overflow: hidden; margin-bottom:8px;">
                    ...
                    <el-select v-model="selctCity" clearable placeholder="请选择" style="float:left;width:100%;">
                    <el-option
        ...
        <p>所在县（区） <span style="color:red">{{toastCheck}}</span></p>
        <el-input class="pop-text ipt-style"
                    placeholder="请输入"
                    v-model="countiesName "
                    v-on:blur="blurCount()"
...
export default {
     ...
     methods:{
           ...
           //判断用户名失去焦点时的用户名是否相同
                    postRequest(url.countiesCheckName, {
                        name: this.countiesName,
                        }).then(data=> {
            ...
            submitAdd(){
                ...
                postRequest(url.countiesInsertCounties, {
                        pageNum: this.currentPage,
                        cityName : this.selctCity , 
                        countiesName : this.countiesName , 
                    }).then(data=> {
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.countiesDeleteCounties, {
                                pageNum: this.currentPage,
          ...
          handleCurrentChange:function(){
            ...
            postRequest(url.countiesFindAllCounties, {
                 pageNum:this.currentPage,
                            //    token:getCookie("token") 
                            }).then(data=> {
                                    console.log("打印的数据",data.data.data)
