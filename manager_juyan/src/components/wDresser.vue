    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>四大金刚</span><span>/跟妆</span>
         </div>
        </div>
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增跟妆</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.makeupName}}</td>
                <td style="text-align: center;">{{item.makeupIntroduce }}</td>
...
export default {
 ...
 /****************************查看详情over********************************************************/             
             handleAvatarSuccess(res, file) {
                ...
                //    let fd = new FormData()
                //    fd.append('file', file)
                //    fd.append('makeupName', this.title)
                //    fd.append('makeupIntroduce', this.textContent)
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.maCheckMakeupName, {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.maUpdateMakeup+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.maUpdateMakeup+"?token="+getCookie("token");
                 console.log(this.list[index].makeupName)
                 console.log(this.list[index].makeupIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].makeupName;
                
                let contentAll = this.list[index].makeupIntroduce.split('`');
              ...
              //  this.textContent5=contentAll[4];
                this.textContent6=contentAll[4];
                if(this.$refs.upload){
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.maInsertMakeup+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.maInsertMakeup+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
     ...
     if(!this.title || !this.textContent1 || !this.textContent2 || !this.textContent3 || !this.textContent4 || !this.textContent6){
                     const h = this.$createElement;
                          this.$notify({
                            title: '登录状态',
                ...
                this.pppss.makeupName=this.title
                // this.pppss.makeupIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5+"`"+this.textContent6
                 this.pppss.makeupIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent6
                ...
                if(this.submitName=="确认修改"){
                    ...
                    this.pppss.pictureKey= this.list[index].pictureKey
                     ...
                     postRequest(url.maUpdateMakeup, {
                                id:this.list[index].id,
                                makeupName:this.title,
                                makeupIntroduce:this.pppss.makeupIntroduce,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.maDeleteMakeup, {
                                 pageNum: this.currentPage, 
                                makeupId:this.list[index].id
                                }).then(data=> {
            ...
            goSearch(){//搜索
                postRequest(url.maFindAllMakeup, {
                   keyword: this.search,
                }).then(data=> {
                        console.log("打印的数据",data.data.data)
