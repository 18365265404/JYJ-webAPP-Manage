    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>摄影旅拍</span><span>/摄影旅拍</span>
         </div>
        </div>
         <!-- 搜索栏 -->
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button v-show="rootObj.insert" style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增摄影旅拍</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.weddingPicturesName }}</td>
                <td style="text-align: center;">{{item.weddingPicturesIntroduce }}</td>
                <td style="text-align: center;">
                    <el-button v-show="rootObj.details" size="mini" style="width:85px; font-size:10px;  margin:5px"  @click="showDetail(index)" class="btn" type="danger">查看详情</el-button>
                </td>
                <td style="text-align: center;">
...
export default {
 ...
 /****************************查看详情over********************************************************/             
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.wpcCheckWeddingPictures, {
                        weddingPicturesName: this.title,
                        id:blurId
                        }).then(data=> {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.wpcUpdateWeddingPictures+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.wpcUpdateWeddingPictures+"?token="+getCookie("token");
                 console.log(this.list[index].weddingPicturesName)
                 console.log(this.list[index].weddingPicturesIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].weddingPicturesName;
                let contentAll = this.list[index].weddingPicturesIntroduce.split('`');
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.wpcInsertWeddingPictures+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.wpcInsertWeddingPictures+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
            ...
            //如果是修改模块，不进行验证上传的图片是否为空     
                ...
                if(!this.title || !this.textContent1 || !this.textContent2 || !this.textContent3 || !this.textContent4 || !this.textContent5|| !this.textContent7 ){
                    ...
                    console.log("this.textContentAll",this.textContent1+this.textContent2+this.textContent3+this.textContent4+this.textContent5+this.textContent6+this.textContent7)
                    
                    return 
                }
                this.pppss.weddingPicturesName=this.title
                this.pppss.weddingPicturesIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5+"`"+"团队（隐藏）"+"`"+this.textContent7
                ...
                if(this.submitName=="确认修改"){
                    ...
                    postRequest(url.wpcUpdateWeddingPictures, {
                                id:this.list[index].id,
                                weddingPicturesName:this.title,
                                weddingPicturesIntroduce:this.pppss.weddingPicturesIntroduce,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.wpcDeleteWeddingPictures, {
                                 pageNum: this.currentPage, 
                                weddingPicturesId :this.list[index].id
                                }).then(data=> {
    ...
    //    console.log(this.checkedCities,"打印的checkedCities值")
      ...
      handleCheckedCitiesChange(value) {
        ...
        this.isIndeterminate = checkedCount > 0 && checkedCount < this.list.length;
            ...
            goSearch(){//搜索
                postRequest(url.wpcFindAllWeddingPictures, {
                   keyword: this.search,
                }).then(data=> {
                        console.log("打印的数据",data.data.data)
