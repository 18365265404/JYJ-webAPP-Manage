    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>独立策划师</span><span>/独立策划师</span>
         </div>
        </div>
         <!-- 搜索栏 -->
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button v-show="rootObj.insert" style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增独立策划师</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.schemeHeadline }}</td>
                <td style="text-align: center;">{{item.schemeContent }}</td>
                <td style="text-align: center;">
                    <el-button size="mini" v-show="rootObj.details" style="width:85px; font-size:10px;  margin:5px"  @click="showDetail(index)" class="btn" type="danger">查看详情</el-button>
                </td>
                <td style="text-align: center;">
        ...
        <div style="width:100%; overflow: hidden; margin-top:8px;">
                    ...
                    <el-select @change="changeCity()" v-model="textContent4" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
        ...
        <div style="width:100%; overflow: hidden; margin-top:8px;">
                    <span style="border:1px solid #cccccc; float:left; height:38px; width:17%; line-height: 38px;text-align: center">选择城市</span>
                    <el-select  v-model="textContent7" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
        ...
        <p>作品风格：</p>
        <div style="width:100%; overflow: hidden; margin-top:8px;">
                    <span style="border:1px solid #cccccc; float:left; height:38px; width:17%; line-height: 38px;text-align: center">选择风格</span>
                    <el-select v-model="textContent5"  clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
                ...
                <el-table
                    ...
                    <template slot-scope="scope">
                        <div style="height:150px;width:300px;">
                                 ...
                                 <video style="width:200px;margin:20px 30px" :src='qiniuUrl+scope.row.pictureKey' controls="controls">
                                            您的浏览器不支持 video 标签。
                                </video>
                        </div>
...
export default {
 ...
 /****************************查看详情over********************************************************/             
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.seCheckSchemeHeadline, {
                        schemeHeadline: this.title,
                        id:blurId
                        }).then(data=> {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.seUpd+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.seUpd+"?token="+getCookie("token");
                 console.log(this.list[index].schemeHeadline)
                 console.log(this.list[index].schemeContent)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].schemeHeadline;
                let contentAll = this.list[index].schemeContent.split('`');
                this.textContent1=contentAll[5];
                this.textContent2=contentAll[1];
                this.textContent3=contentAll[2];
                this.textContent4=contentAll[3].split("-")[0];
                this.textContent5=contentAll[4];
                this.textContent6=contentAll[0];
                this.textContent7=contentAll[3].split("-")[1];
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.seAdd+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.seAdd+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
            ...
            //如果是修改模块，不进行验证上传的图片是否为空     
                ...
                if(!this.title || !this.textContent1 || !this.textContent2 || !this.textContent3 || !this.textContent4 || !this.textContent5 || !this.textContent6 || !this.textContent7){
                    ...
                    console.log("this.textContentAll",this.textContent6+this.textContent2+this.textContent3+this.textContent4+this.textContent5+this.textContent1 +this.textContent7)
                   
                    return 
                }
                this.pppss.schemeHeadline=this.title
                this.pppss.schemeContent=this.textContent6+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"-"+this.textContent7+"`"+this.textContent5+"`"+this.textContent1
                ...
                if(this.submitName=="确认修改"){
                    ...
                    postRequest(url.seUpd, {
                                id:this.list[index].id,
                                schemeHeadline:this.title,
                                schemeContent:this.pppss.schemeContent,
         ...
         // 添加时出现的select 查询风格
                ...
                postRequest(url.citySelCity, {
                    //    token:getCookie("token") 
                            ...
                            console.log("城市修改打印的数据222",data.data.data)
                            let datas=data.data;
                 ...
                 // 添加时出现的select 查询作品分类
                postRequest(url.cowaSelCategroyOfWorks, {
                    moduleName:"独立策划师"
                    //    token:getCookie("token") 
                    }).then(data=> {
                            console.log("作品分类打印数据",data.data.data)
                            let datas=data.data;
