    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>婚礼礼服</span><span>/婚礼礼服</span>
         </div>
        </div>
         <!-- 搜索栏 -->
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增礼服</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.fullDressName }}</td>
                <td style="text-align: center;">{{item.fullDressIntroduce }}</td>
                <td style="text-align: center;">{{item.identifying }}</td>
        ...
        <div style="width:100%; overflow: hidden; margin-bottom:8px;">
                    ...
                    <el-select v-model="textContent2" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
...
export default {
 ...
 /****************************查看详情over********************************************************/             
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.fdaCheckFullDressName, {
                        fullDressName: this.title,
                        id:blurId
                        }).then(data=> {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.fdaUpdateFullDress+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.fdaUpdateFullDress+"?token="+getCookie("token");
                 console.log(this.list[index].fullDressName)
                 console.log(this.list[index].fullDressIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.gender = this.list[index].identifying;
                this.title = this.list[index].fullDressName;
                let contentAll = this.list[index].fullDressIntroduce.split('`');
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.fdaInsertFullDress+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.fdaInsertFullDress+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.gender = "";
            ...
            //如果是修改模块，不进行验证上传的图片是否为空     
                ...
                if(!this.title || !this.textContent1 || !this.textContent2 || !this.textContent3 || !this.textContent4 || !this.textContent5|| !this.gender  ){
                ...
                this.pppss.fullDressName=this.title
                this.pppss.identifying=this.gender
                this.pppss.fullDressIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5
                ...
                if(this.submitName=="确认修改"){
                    ...
                    postRequest(url.fdaUpdateFullDress, {
                                id:this.list[index].id,
                                fullDressName:this.title,
                                fullDressIntroduce:this.pppss.fullDressIntroduce,
                                identifying:this.pppss.identifying,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.fdaDeleteFullDress, {
                                 pageNum: this.currentPage, 
                                 fullDressId :this.list[index].id
                                }).then(data=> {
    ...
    //    console.log(this.checkedCities,"打印的checkedCities值")
      ...
      handleCheckedCitiesChange(value) {
        ...
        this.isIndeterminate = checkedCount > 0 && checkedCount < this.list.length;
            ...
            goSearch(){//搜索
                postRequest(url.fdaFindAllFullDress, {
                   keyword: this.search,
                }).then(data=> {
                        console.log("打印的数据",data.data.data)
