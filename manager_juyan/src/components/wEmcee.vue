    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>四大金刚</span><span>/司仪</span>
         </div>
        </div>
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode()">新增司仪</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.emceeName}}</td>
                <td style="text-align: center;">{{item.emceeIntroduce }}</td>
...
export default {
 ...
 /****************************查看详情over********************************************************/             
             handleAvatarSuccess(res, file) {
                ...
                //    let fd = new FormData()
                //    fd.append('file', file)
                //    fd.append('emceeName', this.title)
                //    fd.append('emceeIntroduce', this.textContent)
        ...
        //    console.log(this.username,"11111111111111111111111111111")
                    postRequest(url.eaCheckEmceeName, {
                        emceeName: this.title,
                        id:blurId
                        }).then(data=> {
            ...
            //修改弹窗出现
             changeTode(index){
                 ...
                 console.log("答应的url",url.server+url.eaUpdateEmcee+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.eaUpdateEmcee+"?token="+getCookie("token");
                 console.log(this.list[index].emceeName)
                 console.log(this.list[index].emceeIntroduce)
                this.showAd=true;
                this.dialogVisible=true;
                this.title = this.list[index].emceeName;
                
                let contentAll = this.list[index].emceeIntroduce.split('`');
            ...
            addTode(){
                ...
                this.submitName="确认增加"
                 console.log("答应的url",url.server+url.eaInsertEmcee+"?token="+getCookie("token"))
                 this.upPictureUrl=url.server+url.eaInsertEmcee+"?token="+getCookie("token");
                this.showAd=true;
                this.dialogVisible=true;
                this.title = "";
                this.textContent1="";
            ...
            submitUpload(index){
                ...
                this.pppss.emceeName=this.title
                this.pppss.emceeIntroduce=this.textContent1+"`"+this.textContent2+"`"+this.textContent3+"`"+this.textContent4+"`"+this.textContent5
                ...
                if(this.submitName=="确认修改"){
                    ...
                    this.pppss.pictureKey= this.list[index].pictureKey
                     ...
                     postRequest(url.eaUpdateEmcee, {
                                id:this.list[index].id,
                                emceeName:this.title,
                                emceeIntroduce:this.pppss.emceeIntroduce,
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.eaDeleteEmcee, {
                                 pageNum: this.currentPage, 
                                emceeId:this.list[index].id
                                }).then(data=> {
        ...
        //控制checkbox复选框
            ...
            goSearch(){//搜索
                postRequest(url.eaFindAllEmcee, {
                   keyword: this.search,
                }).then(data=> {
                        console.log("打印的数据",data.data.data)
