    ...
    <div class="person1 content">
        ...
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style="background: #66B1ff; padding-left: 2%;line-height: 50px;">
                <span class="el-icon-info"></span><span>权限管理</span><span>/子菜单管理</span>
         </div>
        </div>
         <!-- 搜索栏 -->
        ...
        <div class="button-add" style="over background:red; overflow: hidden; margin-top:20px;">
            ...
            <el-button v-show="rootObj.insert" style="float:right; margin-right:5%; margin-bottom:20px" type="primary" @click="addTode('新增')">新增子菜单</el-button>
        </div>
        <!-- 要显示的列表 -->
        ...
        <div class="table" style=" font-size:14px">
		    ...
		    <tr class="thead-tr">
              ...
              <th style="text-align: center;">子菜单名称</th>  
              <th style="text-align: center;">子菜单英文名称</th>  
           ...
           <tbody>
            <tr class="tbody-tr" v-for="(item ,index) of list" :key="item.id">
                ...
                <td style="text-align: center;">{{item.submoduleName }}</td>
                <td style="text-align: center;">{{item.submoduleEnglishName }}</td>
        ...
        <div style="width:100%; overflow: hidden; margin-bottom:8px;">
                    ...
                    <el-select v-model="mainMenu" clearable placeholder="请选择" style="float:left;width:82%;">
                    <el-option
        ...
        <el-input class="pop-text ipt-style"
                    ...
                    clearable>
                    <template slot="prepend">添加子菜单名</template>
        </el-input>

        <span style="color:red">{{toastCheck2}}</span>
        <el-input class="pop-text ipt-style"
                    placeholder="请输入"
                    v-model="submoduleEnglishName "
                    v-on:blur="blurCount2()"
                    style="margin-bottom:6px"
                    clearable>
                    <template slot="prepend">子菜单英文名</template>
        </el-input>
...
export default {
     ...
     methods:{
           ...
           //判断用户名失去焦点时的用户名是否相同
                    ...
                    postRequest(url.saCheckSubmoduleName, {
...
//如果在修改的时候当前值没有进行修改，就不进行判断
           ...
           //判断用户名失去焦点时的用户名是否相同
            ...
            //提交确认添加一条数据
            submitAdd(){
                console.log("fgjsjkfjsdfjksdjfk",this.submoduleName,this.mainMenu,this.submoduleEnglishName)
                if(this.submoduleName=="" || this.mainMenu==""|| this.submoduleEnglishName==""){
                   this.$message({
                    message: '输入框不能为空',
                    type: 'success'
                    });
                ...
                postRequest(url.saInsertSubmodule, {
            ...
            removeTodo(index){
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                        ...
                        type: 'warning'
                             ...
                             postRequest(url.saDeleteSubmodule, {
                                pageNum: this.currentPage,
                                id:this.list[index].id
                                }).then(data=> {
          ...
          handleCurrentChange:function(){
            ...
            postRequest(url.saFindAllSubmodule, {
                 pageNum:this.currentPage,
